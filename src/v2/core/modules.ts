...
/** Coarse rack footprint in grid cells. w = columns, h = rows. */
...
export interface ModuleTypeDef {
  ...
  color: string;
  /**
   * Footprint in coarse rack cells. One cell is RACK_COL_PX wide × RACK_ROW_PX tall.
   * Authors write human-readable sizes like {w:2,h:1} or {w:3,h:2}.
   */
  rackSize: RackSize;
...
const SPLITTER: ModuleTypeDef = {
  ...
  outputs: [
    makeOutput('outA', 'either', 'A', 'Branch A (keeps direction).', { anchor: { x: 1, y: 0.35 }, side: 'right' }),
    makeOutput('outB', 'either', 'B', 'Branch B.', { anchor: { x: 1, y: 0.58 }, side: 'right' }),
    makeOutput('outC', 'either', 'C', 'Branch C.', { anchor: { x: 1, y: 0.80 }, side: 'right' }),
  ],
...
const MIXER: ModuleTypeDef = {
  ...
  inputs: [
    ...
    makeInput('inB', 'either', 'B', 'Input B.', { required: false, anchor: { x: 0, y: 0.50 }, side: 'left' }),
    makeInput('inC', 'either', 'C', 'Input C.', { required: false, anchor: { x: 0, y: 0.65 }, side: 'left' }),
    makeInput('inD', 'either', 'D', 'Input D.', { required: false, anchor: { x: 0, y: 0.80 }, side: 'left' }),
  ],
  outputs: [makeOutput('out', 'either', 'OUT', 'Combined signal.', { anchor: { x: 1, y: 0.60 }, side: 'right' })],
  defaultSettings: {},
  settingsSpec: [],
  process: (ctx, inputs) => {
...
const ROUTER: ModuleTypeDef = {
  ...
  outputs: [
    makeOutput('outA', 'either', 'A', 'Route A.', { anchor: { x: 1, y: 0.35 }, side: 'right' }),
    makeOutput('outB', 'either', 'B', 'Route B.', { anchor: { x: 1, y: 0.58 }, side: 'right' }),
    makeOutput('outC', 'either', 'C', 'Route C.', { anchor: { x: 1, y: 0.80 }, side: 'right' }),
  ],
